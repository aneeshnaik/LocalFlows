...
def calc_S(q):
    ...
    phi_min = -pi / 25
    phi_max = pi / 25
    phi_edges = torch.linspace(phi_min, phi_max, N_phi + 1)
    phi_cens = 0.5 * (phi_edges[1:] + phi_edges[:-1])
    dphi = np.diff(phi_edges)[0]

    # stack into arrays
    phi_tiled = phi_cens[None].tile([N_pts, 1])
...
with np.errstate(divide='ignore', invalid='ignore'):
    res = np.divide((f_model - f_true), f_true)
